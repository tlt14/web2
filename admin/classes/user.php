...
<?php
	class user
	{
		...
		{
			$query = "SELECT * FROM tbl_khachhang, tbl_vaitro WHERE tbl_khachhang.VaiTro = tbl_vaitro.MaVaiTro";
		...
		{
			$query = "SELECT COUNT(*) as total_rows FROM tbl_khachhang  ";
			$result = $this->db->select($query);
			return $result;
		}

		public function getUserByName($name){ //Dùng để sửa
			$query = "SELECT * FROM tbl_khachhang WHERE TenDangNhap = '$name' ";
			$result = $this->db->select($query);
			return $result;
		}
