	...
	class product
	{
		...
		public function show_product()// Show product ,có tìm kiếm theo tên, kết quả tìm kiếm phân trang
		{
			...
			if (isset($_GET['nameSearch']) && !empty($_GET['nameSearch']) ){
				...
				$query = "SELECT * FROM tbl_sanpham, tbl_loaisanpham WHERE tbl_sanpham.LoaiSanPham = tbl_loaisanpham.MaLoai AND tbl_sanpham.TrangThaiSanPham = '1' 
			    ...
			    $query = "SELECT * FROM tbl_sanpham, tbl_loaisanpham WHERE tbl_sanpham.LoaiSanPham = tbl_loaisanpham.MaLoai AND tbl_sanpham.TrangThaiSanPham = '1' ORDER BY MaSanPham DESC LIMIT $tungTrang, $sanPhamTungTrang "; //DESC: sản phẩm mới nhất sẽ lên đầu danh sách
			} 
		...
		public function getAllProduct() //Dùng cho phân trang,...
		{
			...
			if (isset($_GET['nameSearch']) && !empty($_GET['nameSearch']) ){
				...
				$query = "SELECT * FROM tbl_sanpham, tbl_loaisanpham WHERE tbl_sanpham.LoaiSanPham = tbl_loaisanpham.MaLoai AND tbl_sanpham.TrangThaiSanPham = '1' 
					...
					$query = "SELECT * FROM tbl_sanpham, tbl_loaisanpham WHERE tbl_sanpham.LoaiSanPham = tbl_loaisanpham.MaLoai AND tbl_sanpham.TrangThaiSanPham = '1' ORDER BY maSanPham DESC "; //DESC: sản phẩm mới nhất sẽ lên đầu danh sách
			}
